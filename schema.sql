-- ============================================================
-- RESET COMPLETO DO BANCO — APAGA TUDO E RECRIA DO ZERO
-- ============================================================
...
-- CORREÇÃO: removido "row_security = off" que causava o erro de sintaxe.
-- SET search_path = public é suficiente e válido.
...
-- ============================================================
-- RPC: create_agendamento_rpc
-- Insere um agendamento de forma atomica após checar sobreposicao
-- Entrada: p_id_barbeiro uuid, p_data_hora timestamptz, p_servicos jsonb (array [{"id":"uuid","preco":123}]), p_observacao text, p_preco_total numeric
-- Retorna: id do agendamento inserido
-- ============================================================
...
AS $$
DECLARE
  v_usuario UUID := usuario_id();
  v_total_dur INTEGER := 0;
...
BEGIN
  ...
  -- calcula duração total dos serviços passados
  IF p_servicos IS NULL OR jsonb_array_length(p_servicos) = 0 THEN
    v_total_dur := 30;
  ELSE
    SELECT COALESCE(SUM(s.duracao_min), 0) INTO v_total_dur
    FROM tb_servico s
    JOIN (
      SELECT (elem->>'id')::UUID AS sid
      FROM jsonb_array_elements(p_servicos) elem
    ) AS ids ON ids.sid = s.id;
    IF v_total_dur = 0 THEN
      v_total_dur := 30;
    END IF;
  END IF;

  -- verifica conflitos: novo intervalo [start, end) vs existentes
  WITH new AS (
    SELECT p_id_barbeiro AS id_barbeiro, p_data_hora AS start, p_data_hora + (v_total_dur * INTERVAL '1 minute') AS end
  ), existing AS (
    SELECT a.id, a.data_hora AS start, (a.data_hora + (COALESCE(SUM(s.duracao_min), 30) * INTERVAL '1 minute')) AS end
    FROM tb_agendamento a
    LEFT JOIN tb_servico_has_tb_agendamento sha ON sha.id_agendamento = a.id
    LEFT JOIN tb_servico s ON s.id = sha.id_servico
    WHERE a.id_barbeiro = p_id_barbeiro
      AND a.status IN ('pendente','confirmado')
    GROUP BY a.id, a.data_hora
  )
  SELECT EXISTS(SELECT 1 FROM existing e, new n WHERE n.start < e.end AND n.end > e.start) INTO v_conflict;

  IF v_conflict THEN
    RAISE EXCEPTION 'Conflito de horário: já existe agendamento neste intervalo.';
  END IF;

  -- inserir agendamento e linhas de serviço atomicalmente
  INSERT INTO tb_agendamento (id_usuario, id_barbeiro, data_hora, observacao, preco_total, created_at, updated_at)
  VALUES (v_usuario, p_id_barbeiro, p_data_hora, p_observacao, p_preco_total, NOW(), NOW())
  RETURNING id INTO v_new_id;

  IF p_servicos IS NOT NULL AND jsonb_array_length(p_servicos) > 0 THEN
    INSERT INTO tb_servico_has_tb_agendamento (id_servico, id_agendamento, preco_servico)
    SELECT (elem->>'id')::UUID, v_new_id, COALESCE(NULLIF((elem->>'preco')::NUMERIC, NULL), s.preco)
    FROM jsonb_array_elements(p_servicos) elem
    LEFT JOIN tb_servico s ON s.id = (elem->>'id')::UUID;
  END IF;
...
GRANT EXECUTE ON FUNCTION create_agendamento_rpc(UUID, TIMESTAMPTZ, JSONB, TEXT, NUMERIC) TO authenticated, anon;
...
-- ============================================================
-- POLICIES: Storage — limpeza prévia
-- ============================================================
...
DROP POLICY IF EXISTS "avatars: atualizar proprio"         ON storage.objects;
DROP POLICY IF EXISTS "avatars: deletar proprio ou admin"  ON storage.objects;

DROP POLICY IF EXISTS "barbers: leitura publica"           ON storage.objects;
DROP POLICY IF EXISTS "barbers: upload apenas admin"        ON storage.objects;
DROP POLICY IF EXISTS "barbers: atualizar apenas admin"     ON storage.objects;
DROP POLICY IF EXISTS "barbers: deletar apenas admin"       ON storage.objects;

DROP POLICY IF EXISTS "gallery: leitura publica"           ON storage.objects;
DROP POLICY IF EXISTS "gallery: upload apenas admin"        ON storage.objects;
DROP POLICY IF EXISTS "gallery: atualizar apenas admin"     ON storage.objects;
DROP POLICY IF EXISTS "gallery: deletar apenas admin"       ON storage.objects;

DROP POLICY IF EXISTS "promocoes: leitura publica"         ON storage.objects;
DROP POLICY IF EXISTS "promocoes: upload apenas admin"      ON storage.objects;
DROP POLICY IF EXISTS "promocoes: atualizar apenas admin"   ON storage.objects;
DROP POLICY IF EXISTS "promocoes: deletar apenas admin"     ON storage.objects;

-- ============================================================
-- POLICIES: Storage — avatars
-- Estrutura de path esperada: avatars/{auth_uid}/{arquivo}
-- ============================================================
...
CREATE POLICY "avatars: deletar proprio ou admin"
...
-- ============================================================
-- POLICIES: Storage — barbers
-- Estrutura de path esperada: barbers/{barbeiro_id}/{arquivo}
-- ============================================================

CREATE POLICY "barbers: leitura publica"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'barbers');

CREATE POLICY "barbers: upload apenas admin"
  ON storage.objects FOR INSERT
  WITH CHECK (
    bucket_id = 'barbers'
    AND is_admin()
...
CREATE POLICY "barbers: atualizar apenas admin"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'barbers' AND is_admin())
  WITH CHECK (bucket_id = 'barbers' AND is_admin());

CREATE POLICY "barbers: deletar apenas admin"
  ON storage.objects FOR DELETE
  USING (bucket_id = 'barbers' AND is_admin());

-- ============================================================
-- POLICIES: Storage — gallery
-- Estrutura de path esperada: gallery/{barbeiro_id}/{arquivo}
-- ============================================================

CREATE POLICY "gallery: leitura publica"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'gallery');

CREATE POLICY "gallery: upload apenas admin"
  ON storage.objects FOR INSERT
  WITH CHECK (
    bucket_id = 'gallery'
    AND is_admin()
...
CREATE POLICY "gallery: atualizar apenas admin"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'gallery' AND is_admin())
  WITH CHECK (bucket_id = 'gallery' AND is_admin());

CREATE POLICY "gallery: deletar apenas admin"
  ON storage.objects FOR DELETE
  USING (bucket_id = 'gallery' AND is_admin());

-- ============================================================
-- POLICIES: Storage — promocoes
-- Estrutura de path esperada: promocoes/{promocao_id}/{arquivo}
-- ============================================================

CREATE POLICY "promocoes: leitura publica"
  ON storage.objects FOR SELECT
  USING (bucket_id = 'promocoes');

CREATE POLICY "promocoes: upload apenas admin"
  ON storage.objects FOR INSERT
  WITH CHECK (
    bucket_id = 'promocoes'
    AND is_admin()
...
CREATE POLICY "promocoes: atualizar apenas admin"
  ON storage.objects FOR UPDATE
  USING (bucket_id = 'promocoes' AND is_admin())
  WITH CHECK (bucket_id = 'promocoes' AND is_admin());

CREATE POLICY "promocoes: deletar apenas admin"
  ON storage.objects FOR DELETE
  USING (bucket_id = 'promocoes' AND is_admin());

-- ============================================================
-- FIM — BANCO LIMPO E PRONTO
-- ============================================================